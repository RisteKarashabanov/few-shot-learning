...
class Merced(Dataset):
    def __init__(self, subset):
        """Dataset class representing Merced dataset
        
        # Arguments:
            subset: Whether the dataset represents the background or evaluation set
        """
        if subset not in ('background', 'evaluation'):
            raise(ValueError, 'subset must be in one of (background, evaluation)')
        self.subset = subset
        ...
        #print("unique_characters: ", self.unique_characters)
        #print("class_name_to_id: ", self.class_name_to_id)
        #print("df: ", self.df)
    ...
    def index_subset(subset):
        ...
        # Returns
            A list of dicts containing information about all the image files in a particular subset of the
            Merced dataset dataset
        """
        images = []
        print('Indexing {}...'.format(subset))
        # Quick first pass to find total for tqdm bar
        subset_len = 0
        for root, folders, files in os.walk(DATA_PATH + '/merced/images_{}/'.format(subset)):
        ...
        #print("{} size: {}".format(subset, subset_len))
        return images
